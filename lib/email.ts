...
export async function sendInviteEmail(
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; background: #111; color: #eee; padding: 32px; border-radius: 12px;">
        <h1 style="color: #4ade80; margin-top: 0;">Welcome to FunBetz</h1>
        <p>An account has been created for you with the username <strong>${username}</strong>.</p>
        <p>Set your password using the button below and you're in. The link expires in <strong>7 days</strong>.</p>
        <a href="${inviteUrl}" style="display: inline-block; margin: 24px 0; padding: 12px 28px; background: #4ade80; color: #111; font-weight: bold; border-radius: 8px; text-decoration: none;">
          Set Your Password
        </a>
        <p style="color: #888; font-size: 13px;">You'll choose your own password &mdash; nobody else ever sees it. If you weren't expecting this invite, you can ignore this email.</p>
        <hr style="border-color: #333; margin: 24px 0;" />
        <p style="color: #555; font-size: 12px;">FunBetz &middot; Fantasy sports for friends</p>
      </div>
    `,
  });
}
